...
int freyja_model__eggv7_import(char *filename)
{
  ...
  for (m = 0; m < mesh_count; m++)
  {
   ...
   for (j = 0; j < bone_count; j++)
   {
     ...
     transT.Add(id, freyjaGetCurrent(FREYJA_BONE));

     freyjaBonePos3f(pos[0], pos[1], pos[2]);
     freyjaBoneFlags1u(0x00);
...
#else
       freyjaBoneAddChild1u(transT[id]);
#endif
     }
   ...
   {
     freyjaIterator(FREYJA_BONE_TAG, tag.Current());
     
     for (slave->Reset(); slave->CurrentExists(); slave->Next())
     {
       freyjaTagAddSlave1u(transT[slave->Current()]);
     }
   }
#endif
