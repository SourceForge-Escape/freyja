...
/* Decomrpession algorithm from DevIL -- couldn't find on msdn it was 404 */
		...
		for (x = 0; x < dds.header.width; x += 4) 
		{
			...
			for (j = 0, k = 0; j < 4; ++j)
			{
				for (i = 0; i < 4; ++i, ++k)
				{
					...
					{
						unsigned int bpp = 4;
						unsigned int bps = (dds.header.width * 
												  dds.header.pixelformat.bits_per_pixel/8);
						unsigned int sz = dds.header.width * dds.header.height;
					  ...
					  unsigned int *w, unsigned int *h, char *bpp)
{
	...
	switch (1)
	{
	default:	// FIXME: won't always be 3 ( 24 bit )
