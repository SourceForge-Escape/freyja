...
int Md3::Load(char *filename)
{
  ...
  fread_int_small(&_num_bone_frames, f);
  printf("Md3::Load> _num_bone_frames = %i\n", _num_bone_frames);

  fread_int_small(&_num_tags, f);
  printf("Md3::Load> _num_tags = %i\n", _num_tags);

  fread_int_small(&_num_meshes, f);
  printf("Md3::Load> _num_meshes = %i\n", _num_meshes);

  fread_int_small(&_max_skins, f);
  printf("Md3::Load> _max_skins = %i\n", _max_skins);

  fread_int_small(&_header_length, f);
  printf("Md3::Load> _header_length = %i\n", _header_length);

  fread_int_small(&_tag_start, f);
  printf("Md3::Load> _tag_start = %i\n", _tag_start);

  fread_int_small(&_surfaces_start, f);
  printf("Md3::Load> _surfaces_start = %i\n", _surfaces_start);

  fread_int_small(&_file_size, f);
  ...
  for (i = 0; i < _num_bone_frames; i++)
  {
    ...
    fread_float_small(&_bone[i].mins[0], f);
    fread_float_small(&_bone[i].mins[1], f);
    fread_float_small(&_bone[i].mins[2], f);
    //fread(&_bone[i].maxs, 12, 1, f);
    fread_float_small(&_bone[i].maxs[0], f);
    fread_float_small(&_bone[i].maxs[1], f);
    fread_float_small(&_bone[i].maxs[2], f);
    //fread(&_bone[i].center, 12, 1, f);
    fread_float_small(&_bone[i].center[0], f);
    fread_float_small(&_bone[i].center[1], f);
    fread_float_small(&_bone[i].center[2], f);
    //fread(&_bone[i].scale, 4, 1, f);
    fread_float_small(&_bone[i].scale, f);
  ...
  for (i = 0; i < _num_tags * _num_bone_frames; i++)
  {
    ...
    fread_float_small(&_tag[i].center[0], f);
    fread_float_small(&_tag[i].center[1], f);
    fread_float_small(&_tag[i].center[2], f);
  ...
  for(i = 0; i < _num_meshes; i++)
  {
    ...
    {
      fread_float_small(&_mesh[i].texel[j].st[0], f);
      fread_float_small(&_mesh[i].texel[j].st[1], f);
    }
    ...
    if (_surfaces_start+_mesh[i].mesh_size != ftell(f))
    {
      printf("\nMd3::Load> WARNING: _mesh[%i] next mesh offset %i != file pos %lu\n", i, 
	     _surfaces_start+_mesh[i].mesh_size, ftell(f));
