...
#else  // Static loaders must include headers here
#include "mtk_pcx.h"
#include "mtk_ppm.h"
#include "mtk_tga.h"
#include "mtk_jpeg.h"
...
int MtkImage::Load(char *filename)
{
  ...
  {
    name = mtk_image_module_names[i];

    sprintf(module_filename, "mtk_%s.so", name);
    sprintf(module_load, "mtk_image__%s_load", name);
    sprintf(module_check, "mtk_image__%s_check", name);
    ...
    else
    {
      ...
      check = (int (*)(FILE *f))dlsym(handle, module_check);
	...
	return -5;
      ...
      loaded = (loaded || (!(*check)(f) && 
			   !(*load)(f, image, &width, &height, &type)));
...
#else
  // Mongoose: Let's use static loaders on the file
  loaded = (loaded || (!mtk_image__tga_check(f) && 
		       !mtk_image__tga_load(f, &image, &w, &h, &type)));

  loaded = (loaded || (!mtk_image__ppm_check(f) &&
		       !mtk_image__ppm_load(f, &image, &w, &h, &type)));

  loaded = (loaded || (!mtk_image__png_check(f) &&
		       !mtk_image__png_load(f, &image, &w, &h, &type)));

  loaded = (loaded || (!mtk_image__jpeg_check(f) &&
		       !mtk_image__jpeg_load(f, &image, &w, &h, &type)));

  loaded = (loaded || (!mtk_image__pcx_check(f) && 
		       !mtk_image__pcx_load(f, &image, &w, &h, &type)));
#endif
...
int MtkImage::Save(char *filename, char *format)
{
  ...
  sprintf(module_filename, "mtk_%s.so", name);
  sprintf(module_save, "mtk_image__%s_save", name);
  sprintf(module_check, "mtk_image__%s_check", name);
  ...
  else
  {
    ...
    save = (int (*)(FILE *f, 
		    unsigned char *image,
		    unsigned int width, unsigned int height, 
		    char type))dlsym(handle, module_save);
    ...
    if (!f)
    {
      perror("MtkImage::Save> File open failed.\n");
      dlclose(handle);
      return -1;
    }
    
    n = (*save)(f, image, width, height, type);
  ...
  //FIXME: No other save support yet
  n = mtk_image__tga_save(f, _image, _width, _height, 1);
  fclose(f);
  return n;
#endif
}
