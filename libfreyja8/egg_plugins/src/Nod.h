...
typedef struct
{
	float	RestTranslate[3];			   /* The centerpoint of the bone in the at-rest 
													pose of the model */
	float	RestMatrixInverse[3][4];	/* The inverse orientation of the 
													bone in the at-rest pose */
	short	SiblingID;					   /* The index of the next "sibling" bone, 
													or -1 if this is the last */
	...
	short	ParentID;					   /* The index of the parent of this bone. 
													The root bone is always defined first, 
													so -1 is only valid for that bone. */
} nod_bone_t;
 ...
 public:
   ...
   * Mongoose - Created
   ------------------------------------------------------*/
	...
	nod_meshgroup_t *MeshGroups();
	nod_bone_t *Bones();

 private:

	byte *_buffer;
	nod_header1_t *_header1;
	nod_material_t *_materials;
	nod_header2_t *_header2;
	nod_bone_t *_bones;
	nod_mesh_t *_meshes;
	nod_vertex_t *_vertices;
	nod_meshgroup_t *_mesh_groups;
	nod_face_t *_faces;
};
#endif
