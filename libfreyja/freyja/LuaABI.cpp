...
int lua_freyjaPython1s(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 3 )
		return 0;
...
int lua_freyjaLuaCommand1s(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 1 )
		return 0;
	...
	freyjaLuaCommand1s(command);
	return 0;
}


int lua_freyjaLuaScript1s(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 1 )
		return 0;
...
int lua_freyjaLuaBindFunc(lua_State *L)
{
#warning "Unsupported function binding."
	freyjaPrintError("Invalid binding %s(), %s:%i", __func__, __FILE__, __LINE__);
#if FIXME
...
lua_freyja_bind_t gLibFreyja_LuaBinds[] = {
	...
	{ "freyjaPython1s", lua_freyjaPython1s },
	...
	{ "freyjaLuaCommand1s", lua_freyjaLuaCommand1s },
	{ "freyjaLuaScript1s", lua_freyjaLuaScript1s },
	{ "freyjaLuaBindFunc", lua_freyjaLuaBindFunc },
	{ NULL, NULL }
};
