...
#warning "Unsupported function binding."
	...
	byteDepth; uint32 = (int)lua_tonumber(L, 3);
	uint32 width = (int)lua_tonumber(L, 4);
	uint32 height = (int)lua_tonumber(L, 5);
	int integer_value = freyjaTextureCreateBuffer(image, byteDepth, uint32, width, height);
	lua_pushnumber(L, integer_value);
	return 1;
#endif // FIXME
...
int lua_freyjaMaterialDisableMultiTexture(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 1 )
		return 0;
...
int lua_freyjaMaterialEnableMultiTexture(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 1 )
		return 0;
...
int lua_freyjaGetMaterialMultiTextureEnabled(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 1 )
		return 0;
	...
	index_t material = (int)lua_tonumber(L, 1);
	int integer_value = freyjaGetMaterialMultiTextureEnabled(material);
	lua_pushnumber(L, integer_value);
	return 1;
}


int lua_freyjaMaterialDisableBlending(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 1 )
		return 0;
...
int lua_freyjaMaterialEnableBlending(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 1 )
		return 0;
...
int lua_freyjaGetMaterialBlendingEnabled(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 1 )
		return 0;
	...
	index_t material = (int)lua_tonumber(L, 1);
	int integer_value = freyjaGetMaterialBlendingEnabled(material);
	lua_pushnumber(L, integer_value);
	return 1;
}


int lua_freyjaMaterialMultiTextureId(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 3 )
		return 0;
	...
	index_t material = (int)lua_tonumber(L, 1);
	uint32 texture_unit = (int)lua_tonumber(L, 2);
	int32 id = (int)lua_tonumber(L, 3);
	freyjaMaterialMultiTextureId(material, texture_unit, id);
	return 0;
}


int lua_freyjaGetMaterialMultiTextureId(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 2 )
		return 0;
	...
	index_t material = (int)lua_tonumber(L, 1);
	uint32 texture_unit = (int)lua_tonumber(L, 2);
	int integer_value = freyjaGetMaterialMultiTextureId(material, texture_unit);
	lua_pushnumber(L, integer_value);
	return 1;
}
...
int lua_freyjaGetMeshBoundingSphereRadius(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 1 )
		return 0;
	...
	index_t mesh = (int)lua_tonumber(L, 1);
	vec_t real_value = freyjaGetMeshBoundingSphereRadius(mesh);
	lua_pushnumber(L, real_value);
	return 1;
}


int lua_freyjaGetMeshBoundingSphereCenter(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 4 )
		return 0;
...
int lua_freyjaGetCameraCount(lua_State *L)
{
...
int lua_freyjaGetCameraPos3fv(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 4 )
		return 0;
...
int lua_freyjaGetCameraTarget3fv(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 4 )
		return 0;
...
int lua_freyjaGetCameraUp3fv(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 4 )
		return 0;
...
int lua_freyjaGetCameraRotQuat4fv(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 5 )
		return 0;
...
int lua_freyjaGetCameraNameString(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 1 )
		return 0;
	...
	index_t cameraIndex = (int)lua_tonumber(L, 1);
	const char *string_value = freyjaGetCameraNameString(cameraIndex);
	lua_pushstring(L, string_value);
	return 1;
}
...
int lua_freyjaGetCameraMetadata(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 1 )
		return 0;
	...
	index_t camera = (int)lua_tonumber(L, 1);
	const char *string_value = freyjaGetCameraMetadata(camera);
	lua_pushstring(L, string_value);
	return 1;
}
...
int lua_freyjaGetCameraTrackCount(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 1 )
		return 0;
	...
	index_t camera = (int)lua_tonumber(L, 1);
	int integer_value = freyjaGetCameraTrackCount(camera);
	lua_pushnumber(L, integer_value);
	return 1;
}
...
int lua_freyjaGetCameraKeyframeCount(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 2 )
		return 0;
	...
	index_t camera = (int)lua_tonumber(L, 1);
	index_t track = (int)lua_tonumber(L, 2);
	int integer_value = freyjaGetCameraKeyframeCount(camera, track);
	lua_pushnumber(L, integer_value);
	return 1;
}


int lua_freyjaGetCameraKeyframeTime(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 3 )
		return 0;
	...
	index_t track = (int)lua_tonumber(L, 2);
	index_t key = (int)lua_tonumber(L, 3);
	vec_t real_value = freyjaGetCameraKeyframeTime(camera, track, key);
	lua_pushnumber(L, real_value);
	return 1;
}
...
int lua_freyjaGetCameraKeyframeMetadata(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 3 )
		return 0;
	...
	index_t track = (int)lua_tonumber(L, 2);
	index_t key = (int)lua_tonumber(L, 3);
	const char *string_value = freyjaGetCameraKeyframeMetadata(camera, track, key);
	lua_pushstring(L, string_value);
	return 1;
}
...
int lua_freyjaGetCameraKeyframePos3fv(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 6 )
		return 0;
...
int lua_freyjaGetCameraKeyframeRotQuat4fv(lua_State *L)
{
	int stack_count = lua_gettop(L);
	if ( stack_count < 7 )
		return 0;
	...
	index_t key = (int)lua_tonumber(L, 3);
	vec4_t wxyz;
	wxyz[0] = lua_tonumber(L, 4);
	wxyz[1] = lua_tonumber(L, 5);
	wxyz[2] = lua_tonumber(L, 6);
	wxyz[3] = lua_tonumber(L, 7);
...
lua_freyja_bind_t gLibFreyja_LuaBinds[] = {
	...
	{ "freyjaMaterialDisableMultiTexture", lua_freyjaMaterialDisableMultiTexture },
	{ "freyjaMaterialEnableMultiTexture", lua_freyjaMaterialEnableMultiTexture },
	{ "freyjaGetMaterialMultiTextureEnabled", lua_freyjaGetMaterialMultiTextureEnabled },
	{ "freyjaMaterialDisableBlending", lua_freyjaMaterialDisableBlending },
	{ "freyjaMaterialEnableBlending", lua_freyjaMaterialEnableBlending },
	{ "freyjaGetMaterialBlendingEnabled", lua_freyjaGetMaterialBlendingEnabled },
	{ "freyjaMaterialMultiTextureId", lua_freyjaMaterialMultiTextureId },
	{ "freyjaGetMaterialMultiTextureId", lua_freyjaGetMaterialMultiTextureId },
	...
	{ "freyjaGetMeshBoundingSphereRadius", lua_freyjaGetMeshBoundingSphereRadius },
	{ "freyjaGetMeshBoundingSphereCenter", lua_freyjaGetMeshBoundingSphereCenter },
	...
	{ "freyjaGetCameraCount", lua_freyjaGetCameraCount },
	...
	{ "freyjaGetCameraRotQuat4fv", lua_freyjaGetCameraRotQuat4fv },
	{ "freyjaGetCameraNameString", lua_freyjaGetCameraNameString },
	{ "freyjaCameraMetadata", lua_freyjaCameraMetadata },
	{ "freyjaGetCameraMetadata", lua_freyjaGetCameraMetadata },
	{ "freyjaCameraCreateTrack", lua_freyjaCameraCreateTrack },
	{ "freyjaGetCameraTrackCount", lua_freyjaGetCameraTrackCount },
	...
	{ "freyjaGetCameraKeyframeCount", lua_freyjaGetCameraKeyframeCount },
	{ "freyjaGetCameraKeyframeTime", lua_freyjaGetCameraKeyframeTime },
	{ "freyjaCameraKeyframeMetadata", lua_freyjaCameraKeyframeMetadata },
	{ "freyjaGetCameraKeyframeMetadata", lua_freyjaGetCameraKeyframeMetadata },
	{ "freyjaCameraKeyframePos3f", lua_freyjaCameraKeyframePos3f },
	{ "freyjaGetCameraKeyframePos3fv", lua_freyjaGetCameraKeyframePos3fv },
	{ "freyjaGetCameraKeyframeRotQuat4fv", lua_freyjaGetCameraKeyframeRotQuat4fv },
