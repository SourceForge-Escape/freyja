...
unsigned char *buffer_from_utpak(FILE *f, unsigned int offset, unsigned int size, unsigned char key)
{
...
unsigned char *UTPackage::GetVFSObject( const char *dir, const char *file )
{
	...
	buffer = buffer_from_utpak(f, obj->offset, obj->size, obj->key);
...
#   else  // HEAP BUFFERS
				unsigned char *buffer = buffer_from_utpak(mStream, off, sz, mKeyXOR);
