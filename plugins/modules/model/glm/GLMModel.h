 ...
 *-- Test Defines -----------------------------------------------
 *           
 * UNIT_TEST_GLMMODEL - Builds GLMModel class as a console unit test 
 *
 *-- History ------------------------------------------------ 
 *
 * 2004.12.22:
 * Mongoose - Created, based on specs from Ste I included his notes as is
 *            except for the first comment so you can read it
 ==========================================================================*/


#ifndef GUARD__FREYJA_MONGOOSE_GLMMODEL_H_
#define GUARD__FREYJA_MONGOOSE_GLMMODEL_H_
...
/********************
 * MDX file format 
 * (typically uses file extension GLX for mesh, and GLA for anim/skeleton file)
 *
 * Notes:
 *	
 *	- All offset fields are relative to the address of the structure they occur
 *	- So far, the only external symbol needed is MAX_QPATH, plus the 
 *    typedefs for vec3_t, vec2_t etc 
 ********************/
...
typedef struct
{
	int offsets;		// variable sized (mdxmHeader_t->numSurfaces), each offset points to a mdxmSurfHierarchy_t below
...
typedef struct {
	...
	int			*childIndexes;	// [mdxmSurfHierarch_t->numChildren] (variable sized)
...
typedef struct {	// added in GLM version 3 for ingame use at Jake's request
	int offsets;		// variable sized (mdxmHeader_t->numSurfaces), each offset points to surfaces below
} mdxmLODSurfOffset_t;
...
typedef struct {
	...
	mdxmTriangle_t *triangles;
	mdxmVertex_t *vertices;
	mdxmVertexTexCoord_t *texcoords;
...
class GLMModel
{
 public:
	...
	/*------------------------------------------------------
	 * Pre  : 
	 * Post : Constructs an object of GLMModel
	 *
	 *-- History ------------------------------------------
	 *
	 * 2004.12.22: 
	...
	~GLMModel();
	/*------------------------------------------------------
	 * Pre  : GLMModel object is allocated
	 * Post : Deconstructs an object of GLMModel
	 *
	 *-- History ------------------------------------------
	 *
	 * 2004.12.22: 
	...
	mdxmHeader_t mMDXMHeader;  /* */

	mdxmHierarchyOffsets_t *mMDXMHierarchyOffsets;

	mdxmSurfHierarchy_t *mMDXMHierarchy;

	mdxmLOD_t *mMDXMLODs;

	mdxmLODSurfOffset_t *mMDXMSurfOffsets;

	mdxmSurface_t *mMDXMSurfaces;

	MDXMMesh_t *mMDXMeshes;

	mdxaHeader_t mMDXAHeader;
