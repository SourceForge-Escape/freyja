...
void eggFunctionBegin(char *func_symbol_name);

void eggFunctionArgFloat(char *arg_symbol_name, float default_value);
...
unsigned int eggGetBoneMesh1u(unsigned int item, unsigned int *value);
/*------------------------------------------------------
 * Pre  : Bone selected
...
unsigned int eggGetBoneMeshNum();
/*------------------------------------------------------
 * Pre  : Bone selected
...
unsigned int eggGetBoneRotate3f(float *x, float *y, float *z);
/*------------------------------------------------------
 * Pre  : Bone selected
...
public:
	...
	unsigned int eggGetBoneMesh(unsigned int item, unsigned int *value);
	/*------------------------------------------------------
	 * Pre  : Bone selected
	 *        Value set to Mesh id of Bone's mesh_list[item]
	 * Post : Returns PLUGIN_ERROR if Bone doesn't exist
	...
	unsigned int eggGetBoneRotate(float *x, float *y, float *z);
	/*------------------------------------------------------
	 * Pre  : Bone selected
	 * Post : Gets current tag's rotation in eular angles
	 *        Returns PLUGIN_ERROR if Bone doesn't exist
	...
	unsigned int eggBonePos(float x, float y, float z);
	/*------------------------------------------------------
	 * Pre  : eggBegin(FREYJA_TAG);
	 * Post : Current tag's origin is set
	 *
	...
	unsigned int eggBoneFlags(unsigned int flags);
	/*------------------------------------------------------
	 * Pre  : eggBegin(FREYJA_TAG);
	 * Post : Current tag's mesh/tag tree flag is set
	 *
	...
	unsigned int eggBoneAddMesh(unsigned int mesh);
	/*------------------------------------------------------
	 * Pre  : eggBegin(FREYJA_TAG);
	 * Post : mesh added to current tag's mesh list
	 *
	...
	unsigned int eggBoneAddSlave(unsigned int tag);
	/*------------------------------------------------------
	 * Pre  : eggBegin(FREYJA_TAG);
	 * Post : mesh added to current tag's slave tag list
	 *
	...
	unsigned int eggBoneRotate(float x, float y, float z);
	/*------------------------------------------------------
	 * Pre  : eggBegin(FREYJA_TAG);
	 * Post : Sets tag rotation with eular angles
	 *
	...
	unsigned int eggMeshTreeBone(unsigned int tag);
	/*------------------------------------------------------
	 * Pre  : eggBegin(FREYJA_MESHTREE_ANIM_FRAME);
	 * Post : Appends tag to mesh tree frame
	 *
...
private:
	...
	egg_tag_t *mBone;                    /* Current tag/bolt-on */
